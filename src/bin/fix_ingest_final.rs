...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Full Re-ingest with merchant_country...");

    Command::new("podman").args(["exec", "riskfabric_clickhouse", "clickhouse-client", "--database", "riskfabric", "--query", "DROP TABLE IF EXISTS fact_transactions_bronze_raw"]).status()?;
    Command::new("podman").args(["exec", "riskfabric_clickhouse", "clickhouse-client", "--database", "riskfabric", "--query", "DROP TABLE IF EXISTS fact_transactions_bronze"]).status()?;
    ...
    Command::new("podman").args(["exec", "riskfabric_clickhouse", "clickhouse-client", "--database", "riskfabric", "--query", &create_query]).status()?;
    ...
    println!("Creating final table with country...");
    ...
    Command::new("podman").args(["exec", "riskfabric_clickhouse", "clickhouse-client", "--database", "riskfabric", "--query", "RENAME TABLE fact_transactions_bronze TO fact_transactions_bronze_raw, fact_transactions_bronze_final TO fact_transactions_bronze"]).status()?;

    println!("✅ Done. Transactions ready with country.");
    Ok(())
}
