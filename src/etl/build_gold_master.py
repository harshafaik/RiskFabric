...
def build_gold_master():
    ...
    gold_lf = gold_lf.with_columns([
        pl.col("shared_entity_fraud_rate").fill_null(0.0),
        pl.col("degree").fill_null(0),
        pl.col("suspicious_cluster_member").fill_null(0),
        pl.col("campaign_txn_count").fill_null(0),
        pl.col("campaign_total_amount").fill_null(0.0),
        pl.col("campaign_merchant_diversity").fill_null(0),
    ...
    for p in [tx_path, seq_path, cust_path, merch_path, net_path, camp_path, final_parquet_path]:
        if os.path.exists(p):
            os.remove(p)
    os.rmdir(tmp_dir)
