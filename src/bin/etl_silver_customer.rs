...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let cust_out = Command::new("clickhouse-client").args(["--database", "riskfabric", "--query", cust_query]).output()?;
    ...
    let acc_out = Command::new("clickhouse-client").args(["--database", "riskfabric", "--query", acc_query]).output()?;
    ...
    let status = Command::new("sh")
        .arg("-c")
        .arg(format!("clickhouse-client --database riskfabric --query \"INSERT INTO customer_features_silver FORMAT Parquet\" < {}", temp_path))
        .status()?;
