...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Re-attempting Transaction Ingestion with String buffer...");

    // 1. Drop and recreate with String for timestamp
    Command::new("podman")
    ...
    // 3. Convert to a proper typed table
    println!("Converting to typed table...");
    Command::new("podman")
    ...
    let count = Command::new("podman")
        .args(["exec", "riskfabric_clickhouse", "clickhouse-client", "--database", "riskfabric", "--query", "SELECT count() FROM fact_transactions_bronze"])
        .output()?;
    
    println!("✅ Success! Total transactions in ClickHouse: {}", String::from_utf8_lossy(&count.stdout).trim());
