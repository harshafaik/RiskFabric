...
Calculates precision-recall trade-off operating points for the fraud model. Queries the Gold Parquet snapshot via DuckDB, computes precision-recall curves, and outputs threshold layers at multiple recall-fidelity targets (e.g., 95% precision, 90% recall). These thresholds are consumed by `src/ml/scorer.py` for real-time decision boundaries.

**Consumes:** `models/calibrated_fraud_model_isotonic.pkl`, Gold Parquet snapshot.
**Produces:** Threshold operating point report.
